...
def assert_valid(model, cases):
    for case in cases:
        ...
        tokens = Program.library.tokenize(case)
        for i, action in enumerate(case):
            assert prior[i, action] > -np.inf, (
                "The {}-th action in the "
...
def pre_assert_is_violation(model, cases, prior_class, caller):
    ...
    for i, case in enumerate(cases):
        ...
        assert r1 == r2, make_failed_message(
            caller, i, len(cases), "Both methods should return the same results."
        )
...
def make_batch(model, actions):
    ...
    for i in range(L):
        ...
        finished = np.where(np.logical_and(dangling == 0, lengths == 0), True, False)
...
def find_prior_from_joint(joint_prior: JointPrior, prior_type: Type[Prior]):
    """
    ...
    for prior in joint_prior.priors:
        if isinstance(prior, prior_type):
            found_prior = prior
            break
    assert found_prior is not None, "Prior {} not found in JointPrior object.".format(
        prior_type
    )
    return found_prior
...
def test_domain_range_length_special(model):
    ...
    invalid_cases.append(
        "add,add,mul,mul,cos,mul,add,exp,x1,add,mul,mul,mul,add,mul,sub,x1,log"
    )  # Empirical example of collision
...
def test_multi_discrete():
    ...
    config = load_config(str(Path(__file__).resolve().parents[1] / config))
    config["experiment"]["logdir"] = None  # Turn off saving results
